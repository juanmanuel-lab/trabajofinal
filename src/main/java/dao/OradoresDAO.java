...
public class OradoresDAO {
     public void agregarOrador(Orador orador) {
        String sql = "INSERT INTO oradores (nombre, apellido, tema, fecha_de_alta) VALUES (?, ?, ?, ?)";
    ...
    public Orador obtenerPorId(int id) {
        ...
        try (Connection conn = Conexiondb.conectar();
            ...
            if (rs.next()) {
                ...
                orador.setFechaAlta(rs.getDate("fecha_de_alta"));
                return orador;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void actualizarOrador(Orador orador) {
        String sql = "UPDATE oradores SET nombre = ?, apellido = ?, tema = ?, fecha_de_alta = ? WHERE id_orador = ?";
        try (Connection conn = Conexiondb.conectar();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
